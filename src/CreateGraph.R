...
createGraphForArima <- function(prediction_result, all_coefficients_data, coeLength, predictionTerm, name) {
    for (i in seq(1, 8, by = 1)) {
        ...
        plot <- ggplot() +
            geom_line(aes(x = x1, y = y1), color = "blue", linewidth = 1) +
            geom_point(aes(x = x1, y = y1), color = "blue", size = 4) +
            geom_line(aes(x = x2, y = y2), color = "red", linewidth = 1) +
            geom_point(aes(x = x2, y = y2), color = "red", size = 4) +
            geom_vline(
                xintercept = length(all_coefficient_data) - length(prediction_coefficient_data) + 1,
                linetype = "dotted",
                color = "black",
                linewidth = 1
            ) +
            ...
            theme(
                ...
                legend.position.inside = c(0.005, 0.83),

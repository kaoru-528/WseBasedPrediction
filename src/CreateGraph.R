create_graph_for_swwsp <- function(prediction_result, all_coefficients_data, coe_length, prediction_term, name) {
    for (i in seq(1, 8, by = 1)) {
        ...
        plot <- ggplot() + geom_line(aes(x = x1, y = y1, color = "Actual"), linewidth = 1) + geom_point(aes(x = x1, y = y1, color = "Actual"), size = 4) + geom_line(aes(x = x2, y = y2, color = "Prediction"), linewidth = 1) + geom_point(aes(x = x2, y = y2, color = "Prediction"), size = 4) + geom_vline(xintercept = length(all_coefficient_data) - length(prediction_coefficient_data) + 1, linetype = "dotted", color = "black", linewidth = 1) + scale_color_manual(values = c("Actual" = "blue", "Prediction" = "red")) + labs(x = x_label, y = y_label) + theme(panel.background = element_rect(fill = "transparent",
            color = "black", linewidth = 0.2), panel.grid = element_blank(), text = element_text(size = 24), legend.title = element_blank(), legend.background = element_rect(fill = "white", color = "black", linewidth = 0.2), legend.position = "inside", legend.position.inside = c(0.05, 0.95), legend.justification = c(0, 1), legend.key.size = unit(1, "cm"), legend.key = element_rect(fill = "white", color = "white"))
